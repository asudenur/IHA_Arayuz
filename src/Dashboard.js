...
import { malzemeKayitlariCol as captiveMalzemeKayitlariCol } from './firebase/captivePortalConfig';
import {
  LineChart,
  Line,
  XAxis,
...
const Dashboard = () => {
  ...
  const [connectionStatus, setConnectionStatus] = useState({ iha1: false, iha2: false });
  ...
  useEffect(() => {
    console.log('Bağlantı durumu kontrol ediliyor:', {
      ...
      attitude2: telemetryData.attitude2,
      connectionStatus
    });
    
    // IHA1 bağlantı kontrolü
    if (telemetryData.attitude && !connectionStatus.iha1) {
      console.log('IHA1 bağlandı!');
      setConnectionStatus(prev => ({ ...prev, iha1: true }));
      // Geçici bağlantı bildirimi göster
      const connectionNotif = {
        id: 'iha1-connection',
        type: 'connection',
        message: 'Haberleşme İHA bağlandı!',
        timeout: 5000 // 5 saniye sonra kaybol
      };
      setNotifications(prev => [...prev, connectionNotif]);
      
      // 5 saniye sonra bildirimi kaldır
      setTimeout(() => {
        setNotifications(prev => prev.filter(n => n.id !== 'iha1-connection'));
      }, 5000);
    }

    // IHA2 bağlantı kontrolü
    if (telemetryData.attitude2 && !connectionStatus.iha2) {
      console.log('IHA2 bağlandı!');
      setConnectionStatus(prev => ({ ...prev, iha2: true }));
      // Geçici bağlantı bildirimi göster
      const connectionNotif = {
        id: 'iha2-connection',
        type: 'connection',
        message: 'Lojistik İHA bağlandı!',
        timeout: 5000 // 5 saniye sonra kaybol
      };
      setNotifications(prev => [...prev, connectionNotif]);
      ...
      // 5 saniye sonra bildirimi kaldır
      setTimeout(() => {
        setNotifications(prev => prev.filter(n => n.id !== 'iha2-connection'));
      }, 5000);
    }
  }, [telemetryData.attitude, telemetryData.attitude2, connectionStatus]);

  // Bildirimleri güncelle
  useEffect(() => {
    // Sadece connection tipindeki bildirimleri koru
    const newNotifications = notifications.filter(n => n.type === 'connection');

    // Diğer uyarıları ekle
    if (telemetryData.battery && telemetryData.battery.voltage > 0 && telemetryData.battery.voltage < 15) {
      newNotifications.push({
        id: 'bat1',
        type: 'battery',
        message: `Haberleşme İHA: Kritik Batarya Seviyesi! (${telemetryData.battery.voltage.toFixed(1)}V)`
      });
    }
    if (telemetryData.status && telemetryData.status.safety_switch === false) {
      newNotifications.push({
        id: 'switch1',
        type: 'switch',
    ...
    if (telemetryData.battery2 && telemetryData.battery2.voltage > 0 && telemetryData.battery2.voltage < 12) {
      newNotifications.push({
        id: 'bat2',
        type: 'battery',
        message: `Lojistik İHA: Kritik Batarya Seviyesi! (${telemetryData.battery2.voltage.toFixed(1)}V)`
      });
    }
    if (telemetryData.status2 && telemetryData.status2.safety_switch === false) {
      newNotifications.push({
        id: 'switch2',
        type: 'switch',
        message: 'Lojistik İHA: Switch Kapalı! Lütfen açınız.'
      });
    }
    setNotifications(newNotifications);
  }, [telemetryData.battery, telemetryData.status, telemetryData.battery2, telemetryData.status2, notifications]);
      ...
      <div style={{ 
        ...
        {notifications.map((notif, idx) => (
          <div
            key={notif.id}
            className={
              notif.type === 'battery' ? 'battery-warning-banner' : 
              notif.type === 'switch' ? 'switch-warning-banner' :
              notif.type === 'connection' ? 'connection-banner' : 'battery-warning-banner'
            }
            style={{ 
              top: `${idx * 48}px`, 
              position: 'absolute', 
