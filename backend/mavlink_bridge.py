...
mav_connection2 = mavutil.mavlink_connection('/dev/ttyUSB2', baud=57600)
...
def should_update(data_type):
    ...
    # Veri tipi last_send_time'da yoksa ekle
    if data_type not in last_send_time:
        last_send_time[data_type] = 0
    ...
    while True:
        try:
            ...
            if msg_type == 'GLOBAL_POSITION_INT':
                ...
                update_and_send_if_changed('position', position_data)
            # Attitude verisi
            ...
            # Sistem durumu
                ...
                safety_switch = bool(msg_dict['base_mode'] & 128)  # MAV_MODE_FLAG_SAFETY_ENABLED = 128
    ...
    while True:
        try:
            ...
            if msg_type == 'GLOBAL_POSITION_INT':
                ...
                update_and_send_if_changed('position2', position_data)
            # Attitude verisi
            elif msg_type == 'ATTITUDE':
                attitude_data = {
                    'roll': math.degrees(msg_dict['roll']),
                ...
                update_and_send_if_changed('attitude2', attitude_data)
            # Batarya verisi
            elif msg_type == 'SYS_STATUS':
                battery_data = {
                    'voltage': msg_dict['voltage_battery'] / 1000.0 if msg_dict['voltage_battery'] != 0 else 0,
                ...
                update_and_send_if_changed('battery2', battery_data)
            # GPS verisi
            elif msg_type == 'GPS_RAW_INT':
                gps_data = {
                    'fix_type': msg_dict['fix_type'],
                ...
                update_and_send_if_changed('gps2', gps_data)
            # Sistem durumu
            elif msg_type == 'HEARTBEAT':
                armed, flight_mode, system_status = get_flight_mode(
                    msg_dict['base_mode'],
                ...
                update_and_send_if_changed('status2', status_data)
            time.sleep(0.01)  # CPU kullanımını azaltmak için kısa bekleme
        except Exception as e:
